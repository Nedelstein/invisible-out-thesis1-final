...
ofxOpenCvDnnObjectDetection::ofxOpenCvDnnObjectDetection()
{
    ...
    peopleImgs.resize(peopleImgSize);
    for(int i = 1; i < peopleImgSize; i++){
//        peopleImgs[i].load("images/people/" + ofToString(i) + ".jpg");
        loader.loadFromDisk(peopleImgs[i], "images/people/" + ofToString(i) + ".jpg");
    }
...
//    counter = 0;
    ...
    toggle = false;
    randFromPeopleImgs = ofRandom(peopleImgSize);
}
ofxOpenCvDnnObjectDetection::~ofxOpenCvDnnObjectDetection()
{
...
//        if person is detected, fill box with image of person
        ...
        if(toggle) {
                if(object.at(i).class_id == 0){
                    peopleImgs[ofRandom(peopleImgSize)].draw(r_scaled);
                }
        } else {
            if(object.at(i).class_id == 0){
                peopleImgs[randFromPeopleImgs].draw(r_scaled);
            }
        }
        
        if(ofGetElapsedTimef() > startTime + 4){
            toggle = !toggle;
            startTime = ofGetElapsedTimef();
            randFromPeopleImgs = ofRandom(peopleImgSize);
        }
